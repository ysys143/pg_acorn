...
- **지속적**: pgvector가 필터드를 의도적으로 단순히(iterative scan) 풀었고 그게 높은 통과율서 붕괴
  (0.22~0.50). 보수적이라 깊게 안 고침 → 갭이 안 닫힌다.
...
- **C0. (전제) 깨끗한 재측정** — same-protocol·median-basis(prepared·literal·median+p99 분리),
  ...
  - **C0b. iterative_scan 정합 베이스라인 (pgvector #678 — 메인테이너 본인 설계)**: 현재 우리 측정은
    `strict_order` + `max_scan_tuples=40000`만(recall 0.22~0.50). 마저 할 것: **`relaxed_order` ×
    max_scan_tuples 스윕 × 현재 pgvector 버전**. **메커니즘 정정**: post-filter 아님 — discarded
    candidate를 entry로 **그래프 재탐색**(HnswSearchLayer)이되 *traversal이 filter-agnostic*이라
    상관필터 passing-cluster로 못 향함(이게 천장 원인). 거대 예산선 exact로 수렴함도 인정.
    **이 정합 베이스라인 없이 "iterative collapse" 주장/이슈 게시 금지** (작성자에게 메커니즘 틀리면 신뢰 붕괴).
